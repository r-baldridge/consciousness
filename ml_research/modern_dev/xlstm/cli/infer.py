...
Command-line interface for generating text with xLSTM models.

Usage:
    python -m xlstm.cli.infer --checkpoint path/to/model.pt --prompt "Hello"
    python -m xlstm.cli.infer --checkpoint path/to/model.pt --interactive
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate text with an xLSTM model",
...
def load_model(checkpoint_path: str, config_path: Optional[str], device: str, dtype: str):
    ...
    if "config" in checkpoint:
        ...
        config = xLSTMConfig(**config_dict) if isinstance(config_dict, dict) else config_dict
    elif config_path:
        import yaml
        with open(config_path, "r") as f:
            config_dict = yaml.safe_load(f)
        config = xLSTMConfig(**config_dict.get("model", config_dict))
    else:
    ...
    logger.info(f"Layer stats: {model.get_layer_stats()}")
...
def interactive_mode(model, tokenizer, args: argparse.Namespace):
    ...
    """
    print("\n" + "=" * 60)
    print("xLSTM Interactive Generation")
